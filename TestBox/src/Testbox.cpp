...
public:
	...
	{
		m_VertexArray.reset(Arklumos::VertexArray::Create());
		...
		m_SquareVA.reset(Arklumos::VertexArray::Create());
		...
		m_Shader.reset(Arklumos::Shader::Create(vertexSrc, fragmentSrc));
		...
		m_FlatColorShader.reset(Arklumos::Shader::Create(flatColorShaderVertexSrc, flatColorShaderFragmentSrc));
		...
		m_TextureShader.reset(Arklumos::Shader::Create(textureShaderVertexSrc, textureShaderFragmentSrc));

		m_Texture = Arklumos::Texture2D::Create("assets/textures/Checkerboard.png");

		std::dynamic_pointer_cast<Arklumos::OpenGLShader>(m_TextureShader)->Bind();
		std::dynamic_pointer_cast<Arklumos::OpenGLShader>(m_TextureShader)->UploadUniformInt("u_Texture", 0);
	}

	void OnUpdate(Arklumos::Timestep ts) override
	{
		if (Arklumos::Input::IsKeyPressed(AK_KEY_LEFT))
		{
			m_CameraPosition.x -= m_CameraMoveSpeed * ts;
		}
		else if (Arklumos::Input::IsKeyPressed(AK_KEY_RIGHT))
		{
			m_CameraPosition.x += m_CameraMoveSpeed * ts;
		}

		if (Arklumos::Input::IsKeyPressed(AK_KEY_UP))
		{
			m_CameraPosition.y += m_CameraMoveSpeed * ts;
		}
		else if (Arklumos::Input::IsKeyPressed(AK_KEY_DOWN))
		{
			m_CameraPosition.y -= m_CameraMoveSpeed * ts;
		}

		if (Arklumos::Input::IsKeyPressed(AK_KEY_A))
		{
			m_CameraRotation += m_CameraRotationSpeed * ts;
		}
		if (Arklumos::Input::IsKeyPressed(AK_KEY_D))
		{
			m_CameraRotation -= m_CameraRotationSpeed * ts;
		}
		...
		Arklumos::Renderer::BeginScene(m_Camera);
		...
		for (int y = 0; y < 20; y++)
		{
			for (int x = 0; x < 20; x++)
			{
				...
				Arklumos::Renderer::Submit(m_FlatColorShader, m_SquareVA, transform);
			}
		}
		m_Texture->Bind();
		Arklumos::Renderer::Submit(m_TextureShader, m_SquareVA, glm::scale(glm::mat4(1.0f), glm::vec3(1.5f)));

		// Triangle
		// Arklumos::Renderer::Submit(m_Shader, m_VertexArray);

		Arklumos::Renderer::EndScene();
	}
...
private:
	...
	Arklumos::Ref<Arklumos::Shader> m_FlatColorShader, m_TextureShader;
	Arklumos::Ref<Arklumos::VertexArray> m_SquareVA;

	Arklumos::Ref<Arklumos::Texture2D> m_Texture;

	Arklumos::OrthographicCamera m_Camera;
